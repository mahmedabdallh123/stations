...
def analyze_failures(df, equipment_name=None):
    ...
    # فلتر حسب المعدة إذا تم تحديدها
    ...
    # 2. أكثر الأعطال تكراراً (تحليل النصوص)
    if "الحدث/العطل" in data.columns:
        # استخراج الكلمات المفتاحية من الأعطال
        all_issues = data["الحدث/العطل"].dropna().astype(str)
        # تبسيط: عد تكرار كل عطل كامل
    ...
    # 3. متوسط الوقت بين الأعطال (MTBF) لكل معدة
    mtbf_results = []
    ...
    data["يوم_الأسبوع"] = data["التاريخ"].dt.day_name()
    weekday_order = ["الاثنين", "الثلاثاء", "الأربعاء", "الخميس", "الجمعة", "السبت", "الأحد"]
    weekday_failures = data["يوم_الأسبوع"].value_counts().reindex(weekday_order).fillna(0).reset_index()
    weekday_failures.columns = ["اليوم", "عدد الأعطال"]
    
    # 6. تحليل التصحيحات (الإجراءات التصحيحية الأكثر استخداماً)
...
def create_failure_charts(analysis):
    """إنشاء رسوم بيانية تفاعلية لتحليل الأعطال"""
    charts = []
    
    # 1. رسم بياني للأعمدة - أكثر المعدات تعطلاً
    if not analysis["failure_rate"].empty:
        fig1 = px.bar(
    ...
    # 2. رسم بياني دائري - نسب الأعطال حسب المعدة
    if not analysis["failure_rate"].empty:
        fig2 = px.pie(
    ...
    # 3. الرسم البياني للتحليل الشهري
    if not analysis["monthly"].empty:
        fig3 = px.line(
            analysis["monthly"],
            x="الشهر",
...
def remove_equipment_from_sheet_data(sheets_edit, sheet_name, equipment_name):
    """حذف جميع السجلات التي تحمل اسم المعدة المحدد"""
    if sheet_name not in sheets_edit:
        return False, "الشيت غير موجود"
    df = sheets_edit[sheet_name]
    if "المعدة" not in df.columns:
...
def failures_analysis_tab(all_sheets):
    ...
    if st.button("🔄 تشغيل التحليل", key="run_analysis", type="primary"):
        with st.spinner("جاري تحليل البيانات..."):
            ...
            charts = create_failure_charts(analysis)
            for chart in charts:
